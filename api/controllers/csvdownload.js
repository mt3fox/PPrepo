...
const csvDownloadController = {
  ...
  downloadSelectedCsv: async (req, res) => {
    try {
      const { invoiceIds } = req.body; // Expecting an array of IDs in the body
      ...
      if (!invoices || invoices.length === 0) {
        return res
          .status(404)
          .json({ error: "No invoices found for the provided IDs" });
      }

      const zip = new JSZip();

      // Generate CSV for each invoice and add to the ZIP
      for (const invoice of invoices) {
        ...
        orderedFields.forEach((key) => {
          if (key === "items") {
            ...
            if (Array.isArray(invoice[key]) && invoice[key].length > 0) {
              invoice[key].forEach((item, itemIndex) => {
                csvData.push([`Item ${itemIndex + 1}`, ""]);
                csvData.push(["Item Description", item.description || ""]);
                csvData.push([
                  "Item Quantity",
                  item.quantity?.toString() || "",
                ]);
            ...
            csvData.push([fieldNameMapping[key] || key, value.toString()]);
          }
        });

        // Create a CSV string
        const csvString = stringify(csvData); // Use stringify here
        const invoiceFileName = `${invoice.invoice_number}.csv`;

        // Add the CSV file to the ZIP
        zip.file(invoiceFileName, csvString);
      }
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=Selected-Invoices.zip`
      );
      res.send(zipContent);
    } catch (error) {
      console.error(error);
      res
        .status(500)
        .json({ message: "Error downloading ZIP", error: error.message });
    }
  },
};
